...
def basic_experiment(x_train, y_train, x_test, y_test, formatted_print=False):
    ...
    # ====== YOUR CODE: ======
    basic_tree = ID3(attributes_names)
    basic_tree.fit(x_train,y_train)
    prediction = basic_tree.predict(x_test)
...
def cross_validation_experiment(x_d, y_d,plot_graph=True):
    ...
    # ====== YOUR CODE: ======
    m_choices = [20*i for i in range(6)]
    assert len(m_choices) >= 5, 'fill the m_choices list with  at least 5 different values for M.'
    kf = KFold(n_splits=5, shuffle=True, random_state=209124296)
    for m in m_choices:
...
def best_m_test(x_train, y_train, x_test, y_test, min_for_pruning):
    ...
    # ====== YOUR CODE: ======
    tree = ID3(attributes_names,min_for_pruning)
...
if __name__ == '__main__':
    ...
    """
    acc = best_m_test(*data_split, min_for_pruning=0)
